...
class ModelTrainer:
    ...
    def train_model(
        ...
        metadata = {
            ...
            # Add recent averages (last 14 days) for more accurate prediction
            "y_recent_mean": float(df['y_original'].tail(14).mean()),
            "y_recent_std": float(df['y_original'].tail(14).std()),
            "y_last_value": float(df['y_original'].iloc[-1]),
